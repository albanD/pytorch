...
class BackwardCFunction(_C._FunctionBase, _ContextMethodMixin, _HookMixin):
    def apply(self, *args):
        ...
        # The user should define either backward or vjp never both.
        # If neither is specified, just call backward that will raise a nice error
        bw_fn = self._forward_cls.backward
        vjp_fn = self._forward_cls.vjp
        if bw_fn is not Function.backward and vjp_fn is not Function.vjp:
            raise RuntimeError("Implementing both 'backward' and 'vjp' for a custom Function "
                               "is not allowed. You should only implemented one of the two.")
        user_fn = bw_fn if bw_fn is not Function.backward else vjp_fn
...
class Function(with_metaclass(FunctionMeta, _C._FunctionBase, _ContextMethodMixin, _HookMixin)):  # type: ignore[misc]
    ...
    def backward(ctx: Any, *grad_outputs: Any) -> Any:
        ...
        output.
        """
        raise NotImplementedError("You must implement either the backward or vjp function for "

...
static PyObject * THPVariable_bitwise_or(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  ...
  if (r.idx == 0) {
    return wrap(dispatch_bitwise_or(self, r.tensor(0)));
  } else if (r.idx == 1) {
    return wrap(dispatch_bitwise_or(self, r.scalar(0)));
  }
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
static PyObject * THPVariable_bitwise_or_(PyObject* self_, PyObject* args, PyObject* kwargs)
{
  ...
  if (r.idx == 0) {
    return wrap(dispatch_bitwise_or_(self, r.tensor(0)));
  } else if (r.idx == 1) {
    return wrap(dispatch_bitwise_or_(self, r.scalar(0)));
  }
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
